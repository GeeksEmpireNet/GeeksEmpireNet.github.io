...
/*[Manager Json Database]*/
exports.addJsonDatabaseToFirestore = functions.runWith(runtimeOptions).https.onRequest((req, res) => {

    var jsonDatabase = fs.readFileSync('database.json');
    var jsonContent = JSON.parse(jsonDatabase);

    const firestore = admin.firestore();

    firestore.collection('Android').doc('FloatIt').set({
        PromotionCodes: jsonContent
    })
        .then(ref => {
    ...
    return "Json Database Imported";
});

/*[Retrieve Promotion Code]*/
exports.takePromotionCodeFloatIt = functions.runWith(runtimeOptions).https.onRequest((req, res) => {

    const ipAddressFastlyIP = req.headers['fastly-client-ip'];
    console.log(`${ipAddressFastlyIP}`);

    const firestore = admin.firestore();

    firestore.collection('Android').doc('FloatIt').get()
        .then(doc => {
            if (!doc.exists) {
                ...
                let aPromotionCode = documentData.PromotionCodes[0];
                console.log('Used Promotion Code ', aPromotionCode);

                var floatItPromotionCodeLink = 'http://GeeksEmpire.net/Projects/Android/FloatIt/FloatItPromotionCode.html?PromotionCode=' + aPromotionCode;
                res.redirect(floatItPromotionCodeLink)
    ...
    return "Taking A Promotion Code";
});
...
async function sendEmailToUsersFloatingWidgets(email) {
    ...
    //mailOptions.text = `Hey ${email || ''}! Welcome to ${APP_NAME}. I hope you will enjoy our service.`;
    var templateHtml = fs.readFileSync('FloatItWidgets.html', "utf-8").toString();
    mailOptions.html = templateHtml
    await mailTransport.sendMail(mailOptions, function (error, info) {
        if (error) {
            return console.log('Error: ' + error + ' | ' + email);
